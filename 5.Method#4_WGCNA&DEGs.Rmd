...
# Method #4. WGCNA + DEGs 
### Analysis applied to mouse gonadal transcriptome during sex differentiation
In this Markdown we will use **all the genes** from RNAseq analysis after bioinformatic analysis and pre-eliminar filtering of the counts dataset. From counts to WGCNA using method #4 of the methodological paper (*unpublished*). 

## 1. Load the data
First, set the working directory, then load the libraries and the 'normalized_counts' dataset from the output of the script preceding this one. 

To apply working directory to all code chuncks of the notebook use the following:
...
#mytable <-read.table("data/normalized_counts.txt", header = T, sep = "\t", dec = ".")


```
## 2. Sample outlier detecction
we produce a sample dendrogram to visualize potential outliers. We set the thershold at height=35 because it removes the highest sample, F2_S2, which coincides with the outlier removed during data filtering before DEGs analysis (script 2.RNAseq_mouse_counts2DEGsOK.R)
...
```
Now, we eliminate the sample and double check. To eliminate the sample we keep all that are grouped at least in groups of 2 and below the height of 35. The variable clust contains the information on which group is each sample. Filter by the ones you want to keep. In this case we keep all that are not listed in group 0.
...
If you have multiple power values with signed R2 above 0.80, then picking the one at an inflection point, in other words where the R2 values seem to have reached their saturation (Zhang and Horvath 2005). You want to a power that gives you a big enough R2 but is not excessively large.
